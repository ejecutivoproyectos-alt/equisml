...
RUTA_ACUSE = os.path.join(
    "datosp",
    "7_ACUSE_preparado_formato_profesional.docx"
)

RUTA_RESUMEN = os.path.join(
    "datosp",
    "6_RESUMEN_EJECUTIVO_AJUSTADO_FUNCIONAL.docx"
)

RUTA_CALENDARIO = os.path.join(
    "datosp",
...
def obtener_formato_por_estilo(parrafo, configuracion):
    try:
        nombre_estilo = parrafo.style.name.strip()
    except Exception:
        nombre_estilo = ""
    ...
    if nombre_estilo in ["Título 2", "Heading 2"]:
        return configuracion["titulo2"]

    if nombre_estilo in ["Título 3", "Heading 3"]:
        return configuracion["titulo3"]

    if nombre_estilo in ["Título 4", "Heading 4"]:
        return configuracion["titulo4"]
...
def reemplazar_en_parrafo(parrafo, reemplazos, configuracion):
    ...
    fuente, tamano, color = obtener_formato_por_estilo(
        parrafo,
        configuracion
    )
    ...
    palabras_negrita = [p for p in palabras_negrita if p]
    ...
    while texto_temp:
        coincidencia = None
        posicion = len(texto_temp)

        for palabra in palabras_negrita:
            idx = texto_temp.find(palabra)
        ...
        if coincidencia is None:
            partes.append((texto_temp, False))
            break

        if posicion > 0:
            partes.append((texto_temp[:posicion], False))

        partes.append((coincidencia, True))
        texto_temp = texto_temp[posicion + len(coincidencia):]

    if not partes:
        partes.append((texto, False))

    for texto_parte, es_negrita in partes:
        run = parrafo.add_run(texto_parte)
        aplicar_fuente(
            run,
            fuente,
            tamano,
...
def procesar_documento(doc, reemplazos, configuracion, lista_conceptos=None):
    for parrafo in doc.paragraphs:
        if lista_conceptos is not None:
            reemplazar_conceptos_en_parrafo(
                parrafo,
                lista_conceptos,
                configuracion
            )
...
def selector_estilos():
    ...
    with col1:
        ...
        tamano_titulo1 = st.number_input(
            "Tamaño Título 1",
            min_value=6,
            max_value=80,
            value=22
        ...
        tamano_titulo2 = st.number_input(
            "Tamaño Título 2",
            min_value=6,
            max_value=80,
            value=18
        ...
        tamano_titulo3 = st.number_input(
            "Tamaño Título 3",
            min_value=6,
            max_value=80,
            value=14
        ...
        color_titulo3 = st.color_picker(
            "Color Título 3",
            "#000000"
        )

    with col2:
        fuente_titulo4 = st.selectbox(
            "Fuente Título 4",
            fuentes,
            index=fuentes.index("Arial") if "Arial" in fuentes else 0
        )

        tamano_titulo4 = st.number_input(
            "Tamaño Título 4",
            min_value=6,
            max_value=80,
            value=12
...
def generar_resumen(
    ...
    plantilla = Document(plantilla_subida)
    contenido = Document(RUTA_RESUMEN)
    ...
    fecha_servicio = (
        f"{fecha_larga_es(fecha_inicio)} al "
        f"{fecha_larga_es(fecha_termino)}"
    )
...
def generar_calendario(
    ...
    plantilla = Document(plantilla_subida)
    contenido = Document(RUTA_CALENDARIO)
...
def mostrar_modulo_documentos_word():
    ...
    conceptos = st.text_area(
        "Conceptos desarrollados para Resumen Ejecutivo",
        value=(
            "Plan integral que permita efectuar con nuevas tecnologías "
            "el monitoreo de rutas logísticas.\n"
            "Digitalización de trámites para reducir tiempos y costos "
    ...
    configuracion = selector_estilos()
    ...
    with col_boton1:
        boton_acuse = st.button(
            "Generar Acuse",
            use_container_width=True
        )

    with col_boton2:
        boton_resumen = st.button(
            "Generar Resumen Ejecutivo",
            use_container_width=True
        )

    with col_boton3:
        boton_calendario = st.button(
            "Generar Calendario",
            use_container_width=True
        )
    ...
    if boton_acuse:
        ...
        elif not os.path.exists(RUTA_ACUSE):
            st.error(f"No se encontró el archivo: {RUTA_ACUSE}")

        elif not empresa_recibe or not empresa_brinda or not nombre_programa:
            st.error("Completa todos los campos.")

        else:
            salida = generar_acuse(
                plantilla_subida,
                empresa_recibe,
                empresa_brinda,
                nombre_programa,
    ...
    if boton_resumen:
        ...
        if plantilla_subida is None:
        ...
        elif not os.path.exists(RUTA_RESUMEN):
            st.error(f"No se encontró el archivo: {RUTA_RESUMEN}")
            ...
            st.success("Resumen Ejecutivo generado correctamente.")
    ...
    if boton_calendario:
        ...
        if plantilla_subida is None:
        ...
        elif not os.path.exists(RUTA_CALENDARIO):
            st.error(f"No se encontró el archivo: {RUTA_CALENDARIO}")

        elif not empresa_recibe or not empresa_brinda or not nombre_programa:
            st.error("Completa todos los campos.")

        else:
            salida = generar_calendario(
                plantilla_subida,
                empresa_recibe,
                empresa_brinda,
                nombre_programa,
            ...
            st.success("Calendario de Trabajo generado correctamente.")

            st.download_button(
                label="Descargar Calendario de Trabajo",
                data=salida,
                file_name="calendario_de_trabajo_generado.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
            )